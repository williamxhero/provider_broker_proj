...
async def cpa(client):
    ...
    async def response(request):
        ...
        if payload.get('stream'):
            ...
            await stream.write(b'data: {"delta":"upstream-one"}\n\n'); await stream.write(b'data: {"delta":"upstream-two"}\n\n'); await stream.write_eof(); return stream
        return web.json_response({'id':'req-test','model':'gpt-5.6-luna','output_text':'hello broker','usage':{'output_tokens':2}})
...
async def test_stream_emits_delta_before_final(client, cpa):
    ...
    assert audit['status']=='completed' and audit['request_id'] and audit['actual_model']=='gpt-5.6-luna'
...
async def test_web_login_uses_session_for_admin_sync(client):
    ...
    assert '/admin/v1/summary' in page and '/admin/v1/providers' in page
    assert 'client-secret' not in page and 'admin-secret' not in page
    for token in ('editProvider','max_parallel','preference','inventory_status','technical_success_rate','avg_ttft_ms','healthClass'):
        assert token in page
    assert "method:'PATCH'" in page and 'name="enabled"' in page and 'name="note"' in page
    for token in ('renderProviders','renderCatalog','renderQuality','暂无数据','className=healthClass','window='):
        assert token in page
    for token in ('requested_model','input_tokens','output_tokens','provider:callprovider.value','status:callstatus.value','next_cursor'):
        assert token in page
    for token in ('syncresult','sync.onclick',"method:'POST'",'/admin/v1/sync','added','updated','offlined','inventory_failures','next.onclick','onchange','/admin/v1/quality?window='):
        assert token in page
    for token in ('official_input_price','official_cache_price','official_output_price','available_provider_count','nd(v.official_input_price)','cancelled','timed_out','transport_failed','protocol_failed','stream_incomplete'):
        assert token in page
    assert 'CPA 管理入口' in page or 'CPA Manager' in page
...
async def test_catalog_is_explicit_and_unknown_models_are_not_priced(client):
    ...
    assert catalog['luna']['intellect'] == 'standard'
    assert catalog['terra']['intellect'] == 'smart'
    assert catalog['sol']['intellect'] == 'expert'
    assert {'official_input_price','official_cache_price','official_output_price','available_provider_count'} <= set(catalog['luna'])
...
async def test_catalog_calibration_round_trips_and_rejects_incomplete(client):
    ...
    assert (await client.patch('/admin/v1/catalog/private-model',headers=headers,json=body)).status == 200
    catalog=(await (await client.get('/admin/v1/catalog',headers=headers)).json())['catalog']
    assert catalog['private-model'] == body | {'available_provider_count':0}
...
async def test_catalog_provider_count_reflects_synced_inventory(client, cpa):
    ...
    assert before['luna']['available_provider_count'] == 0
    ...
    assert after['luna']['available_provider_count'] == 1
...
async def test_summary_counts_only_actual_routable_provider_keys(client, cpa):
    ...
    await client.post('/admin/v1/sync',headers=headers)
    provider=(await (await client.get('/admin/v1/providers',headers=headers)).json())['providers'][0]
    # Synced available Luna is excluded until calibration, then included once.
