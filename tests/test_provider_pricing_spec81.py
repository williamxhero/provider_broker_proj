...
def test_cpa_provider_allowlist_creates_dynamic_cny_combinations(tmp_path):
    ...
    assert {(row["provider_type"], row["model_id"]) for row in rows if row["provider_key"].startswith("key-source:")} == {
    ...
    assert all(
        ...
        for row in rows if row["provider_key"].startswith("key-source:")
    )
